...
public final class QualityControlExecutionLogHelper {
    ...
    /**
     * 历史 execution_log 的标气浓度键（一本账 2026-09-18 定案后停写停读，浓度以 qcm_record 表列为准）。
     * 常量仅为 {@link #metricsForReport} 剥离存量行残留键保留，防止旧浓度值混进报表指标。
     */
    public static final String STD_GAS_CONCENTRATION_KEY = "stdGasConcentration";

    /** {@link #STD_GAS_CONCENTRATION_KEY} 的配套单位键（同上：停写停读，仅存量指标剥离用）。 */
     ...
     * 一本账 2026-09-18 定案：浓度不进 execution_log（stdGas 两键停写，qcm_record 表列受理定格为唯一真相源）。
     */
    public static String toExecutionLogJson(
            Map<String, Object> params,
            Object resultBody,
